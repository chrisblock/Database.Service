...
{
	public class MySqlTypeNameMapper : ITypeNameMapper
	{
		...
		public Type GetType(DatabaseType databaseType, string mySqlTypeName)
		{
			Type result;

			if (TypeMapping.TryGetValue(mySqlTypeName, out result) == false)
			{
				throw new ArgumentException(String.Format("Type '{0}' is not a recognized MySQL system type.", mySqlTypeName));
			}
